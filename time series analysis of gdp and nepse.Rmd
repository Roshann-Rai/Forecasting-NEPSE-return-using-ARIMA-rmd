...
options(repr.plot.width = 16.8, repr.plot.height = 6.2)
...
nepse_df %>%
  ...
  datatable(
    ...
    caption = "Table 1: NEPSE Yearly Index",
    options = list(columnDefs = list(list(className = "dt-center", targets = 0:1)))
  )
```
...
return_rate_plot <- ggplot() +
  ...
  geom_line(return_rate, mapping = aes(x = year, y = gdp_growth_rate), color = "#1930A6", size = 0.7, alpha = 0.6) +
  geom_line(return_rate, mapping = aes(x = year, y = nepse_annual_return), color = "#E00D19", size = 0.7, alpha = 0.6) +
